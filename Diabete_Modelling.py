...
# Business Problem
##############################
# Developing a machine learning model that can predict whether people have diabetes
# when their characteristics are specified is required. You are expected to perform the necessary
...
# Pima Indian women aged 21 years and older living in the city of Phoenix,
# the 5th largest city in the State of Arizona, USA data used for a diabetes survey conducted on a sample of people with diabetes.
# It consists of 768 observations and 8 numeric independent variables.
...
# Insulin: Insulin.
# BMI: Body mass index.
# DiabetesPedigreeFunction: A function that calculates the probability of having diabetes based on our ancestry.
# Age: Age (years)
# Outcome: Information on whether the person has diabetes. Have the disease (1) or not (0)
...
df=pd.read_csv("DataScience/datasets/diabetes.csv")
...
#There are no missing values, but there are values in the dataset that are filled with 0.
#These are with nan replaced and then with median refilled.
